...
- **Fire Exit Door (WEST)** — Aqara door/window sensor T1 (`0x54ef4410014ae8c0`, added 2026-06-02) → `binary_sensor.0x54ef4410014ae8c0_contact` + battery/voltage/linkquality (entity_ids ieee-based; device friendly_name renamed). **Battery installed 2026-06-02** (auto-recorded by the standing rule). LQI ~180. **Open alert:** `automation.fire_exit_door_west_opened_alert` — on open, immediate time-sensitive push to BOTH `mobile_app_julian` + `mobile_app_noddy_iphone` with live CCTV `camera.pier_facing_bags_cctv` attached (iOS: pull down notification for live; tap → CONTROL dashboard CCTV). If live-in-notification is laggy, switch the camera to the Frigate sibling `camera.pier_facing_bags`.
